...
namespace Roadkill.Core.Database.LightSpeed
{
    public class LightSpeedRepository : IRepository
    {
       ...
       public Competition GetCompetitionById(int id)
        ...
        public IEnumerable<Course> FindCoursesByPageId(int pageId)
        {
            using (var uow = new UnitOfWork(connectionString))
            {
                // get all course pages with page Id
                var coursePages = uow.CoursePages.Where(x => x.PageId == pageId);

                if (coursePages != null)
                {
                    using (var uow2 = new UnitOfWork(connectionString))
                    {
                        List<explik_course> courses = new List<explik_course>();
                        foreach (var coursePage in coursePages)
                        {
                            explik_course course = uow2.Courses.SingleOrDefault(x => x.Id == coursePage.CourseId);
                            if (course != null)
                            {
                                courses.Add(course);
                            }
                        ...
                        return FromEntity.ToCourseList(courses);
                    }
                }
